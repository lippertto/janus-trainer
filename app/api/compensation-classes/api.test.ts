...
describe('/compensation-classes', () => {
  test('created entities can be retrieved', async () => {
    ...
    const createResponse = await superagent.post(`${SERVER}/api/compensation-classes`).send(createRequest);
    const createdClass = createResponse.body as CompensationClassDto;
    expect(createdClass.name).toBe(name);

    const findResponse = await superagent.get(`${SERVER}/api/compensation-classes`);
    const foundClasses = findResponse.body.value as CompensationClassDto[];

    expect(foundClasses).toContainEqual(createdClass);

    // clean up
    const deleteResponse = await superagent.delete(`${SERVER}/api/compensation-classes/${createdClass.id}`);
  ...
  test('contains compensation values if requested', async () => {
    ...
    const createResponse = await superagent.post(`${SERVER}/api/compensation-classes`).send({
    ...
    const findResponse = await superagent.get(`${SERVER}/api/compensation-classes?expand=compensationValues`);
    const foundClasses = findResponse.body.value as CompensationClassDto[];

    expect(foundClasses).toContainEqual({ ...createdClass, compensationValues: [] });

    // create compensation values
    const createValueResponse1 = await superagent
      .post(`${SERVER}/api/compensation-classes/${createdClass.id}/compensation-values`)
    ...
    const cvResponse1 = await superagent.get(`${SERVER}/api/compensation-classes/${createdClass.id}/compensation-values/${createValueResponse1.body.id}`);
    expect(cvResponse1.statusCode).toBe(200);
    const cvResponse2 = await superagent.get(`${SERVER}/api/compensation-classes/${createdClass.id}/compensation-values/${createValueResponse2.body.id}`);
    expect(cvResponse2.statusCode).toBe(200);

    // check if the values are returned with the classes
    const ccResponse = await superagent.get(`${SERVER}/api/compensation-classes/${createdClass.id}?expand=compensationValues`);
    ...
    // check compensation values
    const cvResponse1again = await superagent.get(`${SERVER}/api/compensation-classes/${createdClass.id}/compensation-values/${createValueResponse1.body.id}`).ok(res => res.status === 404);
    expect(cvResponse1again.statusCode).toBe(404);
    const cvResponse2again = await superagent.get(`${SERVER}/api/compensation-classes/${createdClass.id}/compensation-values/${createValueResponse2.body.id}`).ok(res => res.status === 404);
