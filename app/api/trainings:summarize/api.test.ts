  ...
  test("happy case", async () => {
    ...
    try {
      ...
      expect(result.body.value).toContainEqual(
        { trainerId: trainerId1, trainerName: "Test-User Admin", newTrainingCount: 2, approvedTrainingCount: 0}
      );
      expect(result.body.value).toContainEqual(
        { trainerId: trainerId2, trainerName: "Test-User Trainer", newTrainingCount: 1, approvedTrainingCount: 0 }
      );
    } finally {
      // clean up
