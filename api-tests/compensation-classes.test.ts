...
describe('/compensation-classes', () => {
  test('created entities can be retrieved', async () => {
    ...
    const createResponse = await withAdminAuth(
      superagent.post(`${SERVER}/api/compensation-classes`),
    ).send(createRequest);
    const createdClass = createResponse.body as CompensationClassDto;
    expect(createdClass.name).toBe(name);

    const findResponse = await withAdminAuth(
      superagent.get(`${SERVER}/api/compensation-classes`),
    );
    ...
    // clean up
    const deleteResponse = await withAdminAuth(
      superagent.delete(
  ...
  test('contains compensation values if requested', async () => {
    ...
    const createValueResponse1 = await withAdminAuth(
      superagent.post(
    ...
    const createValueResponse2 = await withAdminAuth(
      superagent.post(
        `${SERVER}/api/compensation-classes/${createdClass.id}/compensation-values`,
      ),
    ...
    const cvResponse1 = await withAdminAuth(
      superagent.get(
    ...
    const cvResponse2 = await withAdminAuth(
      superagent.get(
    ...
    const ccResponse = await withAdminAuth(
      superagent.get(
    ...
    // check compensation values
    const cvResponse1again = await withAdminAuth(
      superagent.get(
        `${SERVER}/api/compensation-classes/${createdClass.id}/compensation-values/${createValueResponse1.body.id}`,
      ),
    ).ok((res) => res.status === 404);
    expect(cvResponse1again.statusCode).toBe(404);
    const cvResponse2again = await withAdminAuth(
      superagent.get(
        `${SERVER}/api/compensation-classes/${createdClass.id}/compensation-values/${createValueResponse2.body.id}`,
      ),
    ).ok((res) => res.status === 404);
